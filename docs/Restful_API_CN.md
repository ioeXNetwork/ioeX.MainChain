...
* `/api/v1/transactionpool` : 獲取節點交易池數據
...
* `/api/v1/restart` : 重新啟動節點restful服務
...
* `/api/v1/block/details/height/<height>` : 根據區塊 `height` 獲取區塊詳細信息
...
* `/api/v1/block/details/hash/<hash>` : 根據區塊 `hash` 獲取區塊詳細信息
...
* `/api/v1/transaction/<hash>` : 根據交易 `hash` 獲取區塊所有交易信息
...
* `/api/v1/asset/balances/<addr>` : 根據錢包地址獲取錢包餘額
...
* `/api/v1/asset/utxos/<addr>` : 獲取一個地址所有 `UTXO`
...
* `/api/v1/transaction` : 將簽名後的交易數據廣播至節點
